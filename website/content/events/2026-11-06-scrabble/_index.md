---
title: "Scrabble - How Humans and Computers Think About the Game"
date: 2026-08-23-25T09:00:00+02:00
eventStart: 2026-11-06T18:00:00+02:00
eventEnd: 2026-11-06T20:30:00+02:00
eventLocation: "Veranstaltungsraum, Haus des Engagements, Rehlingstraße 9, 79100 Freiburg"
eventGeoLat: 47.98953
eventGeoLon: 7.83979
meetupLink: https://www.meetup.com/de-DE/rationality-freiburg/
lwLink: https://www.lesswrong.com/groups/fFZZ2Ywzsab86EESY
description: "Where: Haus des Engagements, Rehlingstraße 9, 79100 Freiburg. When: Friday, September 25th 2026 at 18:00 hours CEST."
eventHost:
  - Omar
eventType:
  - presentation
  - exercise
layout: single
outputs:
  - HTML
  - Calendar
---

## Preparation

None, but it won't hurt to learn the [rules of the game](https://www.hasbro.com/common/instruct/Scrabble_(2003).pdf).
If you have a board, please bring it.

## What will we do?

We will shortly introduce the rules of the game, which are quite simple.
After that we'll explore how humans think about the game, when trying to find optimal moves, and compare
this to the approaches programmers took to teach computers to play the game.
We will see why this game is difficult for computers so that they are even today not considerably stronger than the top human players.

At the end, we will have some time for practice by playing the game ourselves.

## Organization

You are worried you have nothing to contribute? No worries! Everyone is
welcome!

There always is a mix of German and English speakers and we configure the
discussion rounds so that everyone feels comfortable participating. The primary
language is English.

This meetup will be hosted by Omar.

There will be snacks and drinks.

We will go and get dinner after the meetup. Anyone who has time is welcome to
join.

![Location (Veranstaltungsraum, Haus des Engagements)](/images/hde-new-building-2.png)

<small>In the above map the location where you should leave your bikes is marked
in blue and the entrance (at the end of the metal ramp) with a red cross.</small>

## Other

[Learn more about us]({{< ref "about" >}}).

![Abstract illustration of people sitting around a table discussing evolution, with a tree of life and DNA motifs](cover.png "People discussing evolution")

<small>Image generated with _Gemini_.</small>
